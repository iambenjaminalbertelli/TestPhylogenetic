...
//===================================================================
//= Application de la méthode NJ pour construire une matrice additive
//===================================================================
...
void NJ(double **D1,double **DA,int n)
{
    ...
    for (n1=n;n1>3;n1--)
    {
        ...
        if (Lii<2*epsilon_ba) Lii=2*epsilon_ba;
        if (Ljj<2*epsilon_ba) Ljj=2*epsilon_ba;
        L=L+Lii+Ljj;
        LP[ii]=0.5*D[ii][jj];
    ...
    if (l1<2*epsilon_ba) l1=2*epsilon_ba;
    if (l2<2*epsilon_ba) l2=2*epsilon_ba;
    if (l3<2*epsilon_ba) l3=2*epsilon_ba;
    L=L+l1+l2+l3;
...
void approx_arb(double **DISS,double **TM,double **TMnew,double **W, int *Iternum, long int *ARETE, double *LONGUEUR,int choix,int *kt,int binaire,int n)
{
    ...
    for(k=1;k<=*Iternum;k++)
    {
        ...
        for(i=1;i<=na1;i++) /*/modified line - june 2005*/
        {
            ...
            if (L[i]<5*epsilon_ba) L[i]=5*epsilon_ba;
    ...
    {
        LONGUEUR[i-1]=L[i];
        if (L[i]<5*epsilon_ba) L[i] = LONGUEUR[i-1] = 5*epsilon_ba;
    }
