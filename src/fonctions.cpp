...
void addLeafAndUpdate(struct InputTree *aTree, int choix){
    ...
    aTree->LONGUEUR[(2*n-2-aTree->kt)-1] = ((aTree->LONGUEUR[choix-1]/2.0)>5*epsilon_ba)?(aTree->LONGUEUR[choix-1]/2.0):5*epsilon_ba;

    aTree->ARETE[2*(2*n-1-aTree->kt)-1] = aTree->ARETE[2*choix-2];
    aTree->ARETE[2*(2*n-1-aTree->kt)-2] = 2*n-aTree->kt;
    aTree->LONGUEUR[(2*n-1-aTree->kt)-1] = ((aTree->LONGUEUR[choix-1]/2.0)>5*epsilon_ba)?(aTree->LONGUEUR[choix-1]/2.0):5*epsilon_ba;
    
    aTree->ARETE[2*choix-1] = n+1;
    aTree->ARETE[2*choix-2] = 2*n-aTree->kt;
    aTree->LONGUEUR[choix-1]= 5*epsilon_ba;
...
void ListeSommets_taille_0(double ** matrix,int * tab_sommet,int size){
    ...
    if(cpt < size){
        ...
        cpt_init = cpt++;
        while(cpt <= size){
            if(matrix[cpt_init][cpt] < epsilon_ba ){
                tab_sommet[cpt] = 1;
                nouveau_cas = 1;
            }
            cpt++;
    ...
    for(i=1;i<=2*size-2-kt;i++){
        ...
        for(j=i+1;j<=2*size-2-kt;j++){
            ...
            if(i!=newNode && j!=newNode){
                if( (fabs(DIST[i][size]-DIST[i][Sdest]-lt)< 3*epsilon_ba) && (fabs(DIST[j][size]-DIST[j][Sdest]-lt)>3*epsilon_ba) ){
                ...
                }else if( (fabs(DIST[i][size]-DIST[i][Sdest]-lt)>3*epsilon_ba) && (fabs(DIST[j][size]-DIST[j][Sdest]-lt)<3*epsilon_ba) ){
...
void applyHGT(double**ref,struct InputTree * aTree,int i,int j){
  ...
  //= copie du contenue de ADD dans DIST
    ...
    if(aTree->degre[nodeToDel] == 3){
        ...
        if(aTree->LONGUEUR[i-1] <= 2*5*epsilon_ba){
            for(p=1;p<=2*aTree->size-2-aTree->kt;p++)
                for(q=p+1;q<=2*aTree->size-2-aTree->kt;q++){
                    if((fabs(DIST[p][q] - DIST[p][aTree->ARETE[2*i-1]] - aTree->LONGUEUR[i-1] - DIST[aTree->ARETE[2*i-2]][q]) < 2*epsilon_ba) ||
                       (fabs(DIST[p][q] - DIST[p][aTree->ARETE[2*i-2]] - aTree->LONGUEUR[i-1] - DIST[aTree->ARETE[2*i-1]][q]) < 2*epsilon_ba) ){
                      if((DIST[p][aTree->ARETE[2*i-1]]+DIST[aTree->ARETE[2*i-2]][q]) <(DIST[p][aTree->ARETE[2*i-2]]+DIST[aTree->ARETE[2*i-1]][q])){
                          aTree->ADD[p][q] = aTree->ADD[q][p] = DIST[p][q] = DIST[q][p] = DIST[p][aTree->ARETE[2*i-1]]+DIST[aTree->ARETE[2*i-2]][q] + 2*5*epsilon_ba;
                      }else{
                          aTree->ADD[p][q] = aTree->ADD[q][p] = DIST[p][q] = DIST[q][p] = DIST[p][aTree->ARETE[2*i-2]]+DIST[aTree->ARETE[2*i-1]][q] + 2*5*epsilon_ba;
                      }
                    }
                }
        }
        ...
        aTree->LONGUEUR[branch2-1]= (aTree->LONGUEUR[i-1]/2.0 > 5*epsilon_ba)?aTree->LONGUEUR[i-1]/2.0:5*epsilon_ba;

        //== 5. branch i is used to connect the other source node (i)
        aTree->ARETE[2*i-1] = aTree->ARETE[2*i-2];
        aTree->ARETE[2*i-2] = nodeToDel;
        aTree->LONGUEUR[i-1]= (aTree->LONGUEUR[i-1]/2.0 > 5*epsilon_ba)?aTree->LONGUEUR[i-1]/2.0:5*epsilon_ba;
        nouveauNoeud = nodeToDel;

        for(p=1;p<=2*aTree->size-2-aTree->kt;p++){
            
            if (fabs(DIST[p][aTree->Root] - DIST[p][Sdestination] - DIST[Sdestination][aTree->Root]) < 3*epsilon_ba){
    ...
    //= noeud de degre > a 3
        ...
        double lt = (aTree->LONGUEUR[i-1]/2.0 >= 5*epsilon_ba)?aTree->LONGUEUR[i-1]/2.0:5*epsilon_ba;
  ...
  //== liste des noeuds sous sDestination
    ...
    for(int i=1;i<2*aTree->size-2-aTree->kt;i++){
      if( fabs(DIST[i][aTree->size]-DIST[i][Sdestination] - DIST[Sdestination][aTree->size]) < 5*epsilon_ba){
...
int isAValidHGT(struct InputTree SpeciesTree,int i, int j){
    ...
    if(fabs(SpeciesTree.ADD[root][d4]-SpeciesTree.ADD[root][d1]-SpeciesTree.ADD[d1][d2]-SpeciesTree.ADD[d2][d3]-SpeciesTree.ADD[d3][d4]) < epsilon_ba)
        return 0;
    if(fabs(SpeciesTree.ADD[root][d2]-SpeciesTree.ADD[root][d3]-SpeciesTree.ADD[d3][d4]-SpeciesTree.ADD[d4][d1]-SpeciesTree.ADD[d1][d2]) < epsilon_ba)
        return 0;
    return 1;
}
...
void findBranch(struct InputTree aTree,int *branch, int * elt){
    ...
    if(elt[0]==1){
        ...
        for(j=1;j<=2*aTree.size-3-aTree.kt;j++){
            if( (fabs(aTree.ADD[aTree.Root][aTree.ARETE[2*j-1]]- max) < 2*epsilon_ba) && (aTree.ADD[aTree.Root][aTree.ARETE[2*j-2]] < aTree.ADD[aTree.Root][aTree.ARETE[2*j-1]]) && (fabs(aTree.ADD[aTree.Root][aTree.ARETE[2*j-1]]+aTree.ADD[aTree.ARETE[2*j-1]][elt[1]] - aTree.ADD[aTree.Root][elt[1]]) < 2*epsilon_ba))
                (*branch) = j;
            if( (fabs(aTree.ADD[aTree.Root][aTree.ARETE[2*j-2]]- max) < 2*epsilon_ba) && (aTree.ADD[aTree.Root][aTree.ARETE[2*j-1]] < aTree.ADD[aTree.Root][aTree.ARETE[2*j-2]]) && (fabs(aTree.ADD[aTree.Root][aTree.ARETE[2*j-2]]+aTree.ADD[aTree.ARETE[2*j-2]][elt[1]] - aTree.ADD[aTree.Root][elt[1]]) < 2*epsilon_ba))
                (*branch) = j;
        }
    }
}
